...
import useInvoiceSetting from './use-invoices.hook';
    ...
    <div className="tw-rounded-[10px] tw-bg-white tw-px-5 tw-py-8">
      ...
      <div className="tw-mt-6 tw-flex tw-flex-wrap tw-gap-[17px]">
        ...
        <div className="tw-flex tw-w-full tw-max-w-[347px] tw-flex-col tw-gap-2">
          ...
          <div className="tw-flex tw-items-start tw-rounded-lg tw-bg-[#e4e4e466] tw-p-2.5">
            ...
            <div className="tw-text-xs tw-font-normal tw-not-italic tw-leading-[20.5px] tw-text-text-medium-gray">
              The "Suffix" field complements the invoice number by allowing you to append
              a customizable text
              <span className="tw-text-2xl tw-font-normal tw-not-italic tw-leading-[20.5px] tw-text-black">
                "
              </span>
          ...
          <div className="tw-mt-2">
            <CustomInput
              ...
              placeholder="Enter invoice number suffix"
              type="text"
              isRequired={true}
              register={register}
              errors={errors}
      ...
      <div className="tw-mt-6 tw-flex tw-w-full tw-flex-col tw-gap-2">
        ...
        <div className="tw-flex tw-items-start tw-rounded-lg tw-bg-[#e4e4e466] tw-p-2.5">
          ...
          <div className="tw-text-xs tw-font-normal tw-not-italic tw-leading-[20.5px] tw-text-text-medium-gray">
            By setting an offset, you can begin your invoice numbering sequence at a
        ...
        <div className="tw-mt-2">
          <CustomInput
            name="invoiceNumberOffset"
            placeholder="Enter invoice number offset"
            type="text"
            isRequired={true}
            register={register}
            errors={errors}
      ...
      <div className="tw-mt-4 tw-flex tw-w-full tw-flex-col tw-gap-2">
        ...
        <div className="tw-flex tw-items-start tw-rounded-lg tw-bg-[#e4e4e466] tw-p-2.5">
          ...
          <div className="tw-text-xs tw-font-normal tw-not-italic tw-leading-[20.5px] tw-text-text-medium-gray">
            Whenever the invoice document is booked, the email will be send to the entered
            email addresses with PDF of invoice document
            <span className="tw-text-2xl tw-font-normal tw-not-italic tw-leading-[20.5px] tw-text-black">
              "
            </span>
          </div>{' '}
      ...
      <div className="tw-mt-6">
        ...
        <div className="tw-mt-4">
          <CustomSwitch
            ...
            name="isStatus"
            type="switch"
            checked={attachPdfToggle}
            defaultChecked={attachPdfToggle}
            onChange={(e) => setAttachPdfToggle(e.target.checked)}
          />
        </div>

        {reminderValues.map((reminder, index) => (
          // eslint-disable-next-line react/no-array-index-key
          <div key={index} className="tw-mt-4">
            <CustomInput
              label={`Reminder fee ${index + 1}`}
              name={`reminder_fee_${index + 1}`}
              placeholder={`Enter reminder fee ${index + 1}`}
              type="number"
              value={reminder.fee}
              endIcon={<EuroIcon />}
              className="tw-text-sm tw-font-medium tw-not-italic tw-leading-[17.5px] tw-text-text-dark-gray"
              onChange={(event) => handleReminderChange(event, index)}
            />
          </div>
        ))}
      </div>
      ...
      <div className="tw-mt-6 tw-flex tw-w-full tw-justify-end">
        <CustomButton
          ...
          text="Save"
          onClick={handleSubmit(handleInvoiceSettingSubmit)}
        />
      </div>
    </div>
  );
