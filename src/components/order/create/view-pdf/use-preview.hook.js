...
import { bookAnOrder, getSingleOrder } from '@/provider/features/order/order.slice';
...
function usePreview({ orderId: id }) {
  ...
  useEffect(() => {
    ...
    const numberOfArrays = Math.ceil(
      order && order.orderProducts && order.orderProducts.length / chunkSize
    );
    const newArray = [];
    ...
    for (let i = 0; i < numberOfArrays; i++) {
      newArray.push(
        order.orderProducts &&
          order.orderProducts.slice(i * chunkSize, (i + 1) * chunkSize)
      );
    }
  ...
  useEffect(() => {
    if (order) {
      getSingleTemplateData(order.orderTemplateId);
    }
  }, [order]);
