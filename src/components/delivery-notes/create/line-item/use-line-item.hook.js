...
export default function useLineItem({ handleTabClick, handleTabCompleted }) {
  ...
  const [deliveryNotesProduct, setDeliveryNotesProduct] = useState('');
  ...
  const singleDeliveryNotesHandler = async () => {
    if (Number(searchParams.get('id'))) {
      ...
      setDeliveryNotesProduct(response.payload?.deliveryNotesProducts);
      if (response.payload?.deliveryNotesProducts.length) {
        const result = await singleDocumentrHandler({
          documentProducts: response.payload?.deliveryNotesProducts,
          theDiscount,
          data
        });
        setData(result);
  ...
  const deliveryNotesProducts = lineItemProducts({
    data,
    products: deliveryNotesProduct,
    theDiscount
  });
  ...
  const handleLineItemSubmit = async (e, stayOnThePage) => {
    ...
    const response =
      isSubmit && deliveryNotesProduct.length
        ? await dispatch(updateDeliveryNotes({ payload, id }))
