...
export default function DeliveryNotesAddData({
  handleData = null,
  rowData,
  id,
  action,
          ...
          >
            ...
            >
              ...
              <div className="primary-scroll tw-mt-3 tw-max-h-[79px] tw-overflow-y-auto">
                {deliveryNotesComments?.map((deliveryNotes) => {
                  return (
                    <div
                      className={`tw-mt-3 ${
                        rowData.status.toLowerCase() === 'rejected'
                    ...
                    >
                      <p className="tw-text-xs tw-font-normal tw-not-italic tw-leading-[18px] tw-text-text-medium-gray">
                        ...
                        {deliveryNotes.comment && deliveryNotes.comment.createdByName}
                      </p>
                      <p className="tw-text-xs tw-font-normal tw-not-italic tw-leading-[18px] tw-text-text-medium-gray">
                        <span className="tw-text-xs tw-font-medium tw-not-italic tw-leading-[18px] tw-text-text-black">
                          {' '}
                        ...
                        {formatDate(
                          deliveryNotes.comment && deliveryNotes.comment.updatedAt
                        ) ||
                          formatDate(
                            deliveryNotes.comment && deliveryNotes.comment.createdAt
                          )}
                      </p>
                      <p className="tw-text-xs tw-font-normal tw-not-italic tw-leading-[18px] tw-text-text-medium-gray">
                        {deliveryNotes.comment && deliveryNotes.comment.comment}
                      </p>
                    </div>
                  );
                })}
            ...
            >
              <FileInput module={rowData} moduleName="DELIVERY_NOTES" />
            </div>
            <div
              className={` ${
                rowData.status.toLowerCase() === 'rejected'
            ...
            >
              ...
              <div className="primary-scroll tw-max-h-[100px] tw-overflow-y-auto ">
                ...
                {deliveryNotesHistory &&
                  deliveryNotesHistory?.map((deliveryNotes) => {
                    return (
                      <>
                        {rowData &&
                        rowData?.status !== 'DRAFT' &&
                        deliveryNotes.currentStatus !== 'DRAFT' ? (
                          <p className="tw-mt-2 tw-text-xs tw-font-medium tw-not-italic tw-leading-[18px] tw-text-text-black">
                            ...
                            <span className="tw-mr-[1px] tw-text-text-medium-gray">
                              {deliveryNotes && deliveryNotes.currentStatus
                                ? capitalizeFirstLetter(deliveryNotes.currentStatus)
                                : ''}
                            </span>{' '}
                            to{' '}
                            <span className="tw-mr-[1px] tw-text-text-medium-gray">
                              {deliveryNotes && deliveryNotes.previousStatus
                                ? capitalizeFirstLetter(deliveryNotes.previousStatus)
                                : ''}
                            </span>
                            <br />
                            Status change by:{' '}
                            <span className="tw-mr-[1px] tw-text-text-medium-gray">
                              {deliveryNotes && deliveryNotes.createdByName
                                ? capitalizeFirstLetter(deliveryNotes.createdByName)
                                : ''}
                            </span>{' '}
                            <br />
                            Status change at:{' '}
                            <span className="tw-mr-[1px] tw-text-text-medium-gray">
                              {formatDate(deliveryNotes?.updatedAt) ||
                                formatDate(deliveryNotes?.createdAt)}
                            </span>{' '}
                          </p>
                        ) : null}

                        {deliveryNotes?.convertedFrom && (
                          <p className="tw-mt-2 tw-text-xs tw-font-medium tw-not-italic tw-leading-[18px] tw-text-text-black">
                            Converted From:{' '}
                            <span className="tw-mr-[1px] tw-text-text-medium-gray">
                              {deliveryNotes && deliveryNotes.convertedFrom
                                ? capitalizeFirstLetter(deliveryNotes.convertedFrom)
                                : ''}
                            </span>{' '}
                            <br />
                            Converted To:{' '}
